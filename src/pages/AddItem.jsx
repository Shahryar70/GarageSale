// src/pages/AddItem.jsx - CLEANED VERSION
...
export default function AddItem() {
  const [formData, setFormData] = useState({
    ...
    subCategory: '',
    condition: '',
    itemType: 'Donate', // Default to Donate
    price: '',
    city: '',
    state: '',
    postalCode: '',
  ...
  const handleImageUpload = (e) => {
    ...
    // Create previews (data URLs for display)
    const newPreviews = validFiles.map(file => URL.createObjectURL(file));
    setImagePreviews(prev => [...prev, ...newPreviews]);

    // Convert to base64 WITHOUT data URL prefix (just the base64 part)
    const readerPromises = validFiles.map(file => {
      return new Promise((resolve) => {
        const reader = new FileReader();
        reader.onloadend = () => {
          // Get JUST the base64 part, not the full data URL
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Convert images to semicolon-separated string
      const imagesString = formData.images.join(';');
      
      // Prepare data matching CreateItemDto
      const itemData = {
      ...
      if (result.success) {
        ...
        setTimeout(() => {
          if (result.item?.itemId) {
            navigate(`/items/${result.item.itemId}`);
          } else {
            navigate('/dashboard');
          }
        }, 2000);
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-600">List your item for donation, swap, or sale</p>
          </div>

          {/* Success Message */}
          {success && (
            <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg mb-6">
              <p className="font-medium">Item listed successfully! Redirecting...</p>
            </div>
          )}
            ...
            <div className="space-y-8">
              ...
              <div className="pt-6 border-t">
                <button
                  type="submit"
                  disabled={submitting}
                  className="w-full bg-green-600 text-white py-4 rounded-lg hover:bg-green-700 transition font-medium text-lg disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {submitting ? (
                    ...
                    `List Item for ${formData.itemType === 'Donate' ? 'Donation' : formData.itemType === 'Swap' ? 'Swap' : 'Sale'}`
                  )}
                </button>
                
                <p className="text-center text-gray-500 text-sm mt-4">
                  By listing this item, you agree to our terms of service and confirm this item is yours to list.
                </p>
              </div>
            </div>
          </form>
